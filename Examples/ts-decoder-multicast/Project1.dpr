...
procedure ReceiveMulticast(const sourceUrl: string; OnReceiveData: TProc<TBytes>);
const
  RECV_BUFF_SIZE = 8192;
...
begin
  ...
  try
    ...
    try
      ...
      // BIND
      lSockAddr := StringToSockAddr(sourceUrl);
      ...
      lGroupAddr:= StringToSockAddr(sourceUrl);
            ...
            if assigned((Sender as TTsDecoder).ProgramAssociationTable) then
              ...
              begin
                if assigned((Sender as TTsDecoder).ProgramAssociationTable.Programs[i].Pmt) then
                  ...
                  begin
                    System.WriteLn(Format('Elementary stream type=0x%02x(%s) pid=0x%04x(%d)', [lEsInfo.StreamType.asInt, lEsInfo.StreamType.asString,  lEsInfo.ElementaryPid, lEsInfo.ElementaryPid]));
                    break;
                  end;
              end;
          end;
